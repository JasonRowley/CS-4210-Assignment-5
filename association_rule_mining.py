...
for index, row in df.iterrows():
    ...
    # Set every item to 1 if in transaction else 0
    for item in itemset:
        labels[item] = 1 if item in list(row) else 0
...
for i in rules.index:
    ...
    if num_antecedents == 1:
        ...
        antecedent = ", ".join(str(a) for a in antecedents)
    print(antecedent, end='')
    print(' -> ' + list(rules.loc[i, 'consequents'])[0])
    print('Support: ' + str(rules.loc[i, 'support']))
    ...
    print("Gain in Confidence: " + str(100*(confidence-prior)/prior))
    print()
